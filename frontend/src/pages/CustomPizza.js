...
import pizzaBase from '../assets/images/pizza-base.png';
...
const CustomPizza = () => {
  ...
  const handleTopping = (item) => {
    if (addedToppings.find((itm) => itm.name === item.name)) {
      return setAddedToppings(
        addedToppings.filter((itm) => itm.name !== item.name)
      );
    }
  ...
  const handleCrust = (item) => {
    ...
    if (
      addedCrust.find((itm) => itm.name === item.name) &&
      activeCrust === item.name
    ) {
  ...
  const handleAadToCart = async () => {
    ...
    dispatch(
      addToCart({
        ...
        price: totalPrice,
        image: data,
        description: ' custom pizza made by you!',
        _id: '61e3cbffaba4686f0942e2f2',
        customItem: [...addedToppings, ...addedCrust],
        isCustom: true,
      })
    );
  };
        ...
        <div className="pizz-builder-section">
          <div className="upper-section">
            ...
            <div className="topping">
              <div className="add">ADD TOPPINGS</div>
              <div className="type-section">
                <div
                  className={`type ${showVeg && 'active'}`}
                  onClick={vegTopping}>
                  VEG
                </div>
                <div
                  className={`type ${showNonVeg && 'active'}`}
                  onClick={nonvegTopping}>
                  NON-VEG
                </div>
              </div>
              {allTopping.map((item) => (
                <div
                  ...
                  className={`topping-item ${
                    addedToppings.find((itm) => itm.name === item.name) &&
                    'active'
                  }`}
                  key={item.name}>
                  <p>{item.name}</p>
          ...
          <div className="lower-section">
            ...
            <div className="topping">
              <div className="add">CHOOSE YOUR CRUST</div>

              {crust.map((item) => (
                <div
                  onClick={() => handleCrust(item)}
                  className={`topping-item ${
                    addedCrust.find((itm) => itm.name === item.name) &&
                    activeCrust === item.name &&
                    'active'
